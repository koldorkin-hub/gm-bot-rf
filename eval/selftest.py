...
def check_scenarios(scenarios: list[dict]) -> None:
    ...
    # C 16 и G 17 после двух итераций боевого от 11.09.2026: программа тренировок,
    # автопропуск напоминаний и дубли (день), кардио отдельно и программа главнее сводок
    # (вечер). G09 — регрессия по изометрии от 10.09.2026.
    expected = {"A": 15, "B": 8, "C": 16, "D": 10, "E": 10, "F": 8, "G": 17, "H": 8, "I": 8, "J": 5}
    check(counts == expected, f"состав категорий {counts}, а по плану {expected}")
    check(len(scenarios) == 105, f"сценариев {len(scenarios)}, а должно быть 105")
...
def check_behaviour(scenarios: list[dict]) -> None:
    ...
    # Кардио: добавленное в состав силового дня — провал (вечер 11.09.2026).
    g17 = by_id["G17"]
    run_case(g17, [[{"tool": "training_program", "input": {"action": "set", "title": "Программа",
        "cardio": '[{"name":"эллипс","duration_min":60,"when":"после силовой"}]'}}],
        [{"text": "Добавил эллипс 60 минут отдельным кардио, в силовые дни он не попал."}]], True)
    run_case(g17, [[{"tool": "training_program", "input": {"action": "set", "title": "Программа",
        "days": '[{"day":1,"name":"Грудь","exercises":["жим лёжа","эллипс 60 минут"]}]'}}],
        [{"text": "Добавил эллипс в День 1."}]], False, "training_program.days")
